...
test.describe('Proveedor contratos', () => {
  test('provider can list own contracts and admin is rejected', async ({ request }) => {
    const providerToken = await loginToken(request, PROVIDER_EMAIL, PROVIDER_PASSWORD);
    const providerRes = await request.get('/api/v1/contratos/mios?page=1&limit=20', {
    ...
    const adminRes = await request.get('/api/v1/contratos/mios?page=1&limit=20', {
      headers: { Authorization: `Bearer ${adminToken}` },
    });
    expect(adminRes.status()).toBe(403);
  });

  test('provider can open contratos module from center', async ({ page }) => {
    ...
    await page.getByRole('link', { name: /Mis contratos/i }).first().click();
