...
test.describe('Proveedor participaciones', () => {
  test('provider can list own participations and admin is rejected', async ({ request }) => {
    ...
    const providerRes = await request.get('/api/v1/participaciones/mias?page=1&limit=20', {
    ...
    const adminRes = await request.get('/api/v1/participaciones/mias?page=1&limit=20', {
